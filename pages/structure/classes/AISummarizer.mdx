...
-   A **POST** route (/approval) for changing status (Approval/Disapproval) of summary of a post (user permission required).
...
#### `handleApprovalSummarization(WP_REST_Request $request)`

```php
public static function handleApprovalSummarization(WP_REST_Request $request)
```
...
**Description:**
Handles the summary status of post summary content via the REST API.

_It performs the following tasks:_

-   Verifies the nonce for security.
-   Validates the post_id parameter.
-   Update the post summary status depend on what it receive in `$summaryStatus` parameter.
